...
一个MCP服务器，可以使用OpenAI的gpt-5模型及其强大的Web搜索功能。
...
| `OPENAI_BASE_URL` | 可选 | - | 覆盖 API 基础 URL（例如：`https://api.openai.com/v1`） |
...
要从OpenAI API使用gpt-5模型，您的帐户需要具有对gpt-5的访问权限。
如果您向此MCP注册了尚未启用gpt-5的API密钥，则调用将导致错误。
